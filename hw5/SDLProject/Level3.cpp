...
void Level3::initialise()
{
    ...
    if (!m_state.jump_sfx)  SDL_Log("shoot.mp3 not loaded:  %s", Mix_GetError());
    if (!m_sfxEnemyDie)     SDL_Log("damage.mp3 not loaded: %s", Mix_GetError());
    if (!m_sfxPlayerDie)    SDL_Log("explosion.mp3 not loaded: %s", Mix_GetError());
}


void Level3::spawn_enemy()
{
    Entity e;
    e.textureID = Utility::LoadTexture("assets/textures/enemy3.png");
...
void Level3::process_input(const SDL_Event &e)
{
...
void Level3::update(float dt)
{
    ...
    m_spawn_timer -= dt;
    if (m_spawn_timer <= 0){ m_spawn_timer = 0.9f; spawn_enemy(); }

    for (auto &e : m_enemies)
    {
        glm::vec3 dir = m_state.player->position - e.position;
        float len2 = glm::length2(dir);
        if (len2 > 0.01f) dir /= sqrt(len2);
        e.velocity = dir * 2.8f;
        e.Update(dt);
    }
    ...
    if (m_kills >= m_targetKills) switch_scene(s_win);
}

void Level3::render(ShaderProgram *p)
{
    ...
    Utility::draw_text(p, m_font,
        "Level: 3  Kills: " + std::to_string(m_kills) +
